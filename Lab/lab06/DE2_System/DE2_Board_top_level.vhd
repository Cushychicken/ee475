...
-- Description:      Top Level VHDL Template File for DE2 Board
--                   This file contains the VHDL Entity for all the physical signals
--                   on the board that can be connected by the Cyclone II FPGA
-- Dependencies:     None
-- Revision:         1.01
-- Revision 1.00 -   File Created
--          1.01 -   Updated for Quartus II 10.0
-- 
...
--                   Verify by using the Pin Planner and making sure they have been loaded (Assignments->Pin Planner)
--                   
...
--                   	1. Set nCEO by double clicking on the value setting and selecting "Use as regular I/O" in the pull down menu.
--                         (this pin is also used by IRDA_TXD)
--                      2. Click OK
--
-- Note 5:           The instructions for inserting the Nios II Component is found on 
--                   lines starting at ~220 and ~232
--
--
----------------------------------------------------------------------------------
library IEEE;
   ...
   ---------------------------------------------------------------
    ...
    u0 : component nios_qsys
        port map (
            ...
            leds_export     => LEDR(17 DOWNTO 0),		       --     leds.export
            switches_export => SW(17 DOWNTO 0), 				 -- switches.export
            ...
            sdram_cs_n      => DRAM_CS_N,					       --         .cs_n
				lcd_RW          => LCD_RW,          				 --      lcd.RW
            lcd_E           => LCD_EN,           				 --         .E
            lcd_data        => LCD_DATA,				          --         .data
            lcd_RS          => LCD_RS,  					       --         .RS
				rs232_txd       => UART_TXD,					       --    rs232.txd
            rs232_rxd       => UART_RXD,				          --         .rxd
				pwm1_export     => GPIO_1(1),     					 --     pwm1.export
            pwm2_export     => GPIO_1(3),    					 --     pwm2.export
            pwm3_export     => GPIO_1(5)     					 --     pwm3.export
        );	  
		  
	clockPLL_inst : clockPLL 
		PORT MAP (
			inclk0	 => CLOCK_50,
			c0	 		 => clk_nios,
			c1	 		 => DRAM_CLK
...
--	LCD_RS   <= '1';  -- Command/Data Select '0' = Command, '1' = Data
	...
	-- Expansion Header
	GPIO_0 <= (others => '0');  -- JP1
	GPIO_1(31 DOWNTO 6) <= (others => '0');  -- JP2
	GPIO_1(4) <= '0';
	GPIO_1(2) <= '0';
	GPIO_1(0) <= '0';
